...
graph <- function(graph){
  knitr::include_graphics(here("images",graph))
}
```
...
#includes overview graph from initial report in knitted document
graph("ssl scores overview graph-1.png")
...
#knitr::include_graphics(here("images","scores by age-1.png"))
graph("scores by age-1.png")
graph("ssl scores by age group-1.png")
graph("ssl scores by age group-2.png")
...
From the second and third graphs, which depict current age of Subjects, we note that age group with the highest scores is under 20! As subjects get older, their risk for becoming party to violence decreases. These observations lines up with age demographics of violent crime offenders; past offenders are significantly less likely to commit crimes as they age, particularly coming out of long periods of incarceration.

## Accuracy and Age

```{r Accuracy, echo=FALSE, out.width = '100%'}

graph("oldies visualization-1.png")
...
```{r Gang Affiliation, echo=FALSE, out.width = '100%'}
graph("age & gang affiliation-2.png")
graph("age & gang affiliation-1.png")
...
```{r Race, echo=FALSE, out.width = '100%'}
graph("by race-1.png")
graph("by race-2.png")
